# Chain of Responsibility（チェーンオブレスポンシビリティ）パターン ― 要求を処理できる担当者まで順に回す

次のような経験をしたことはありませんか？

> 処理を行う担当者や条件を判定する分岐処理を 1 つのメソッドにまとめて書いていたら、担当者の種類や条件が増えるたびに、その分岐へ手を入れる羽目になった。おまけに、条件を書く順番を 1 つ間違えただけで、本来とは違う担当者が処理してしまうこともあった。
...
> 現在、社員が経費を申請すると、必ず主任が内容を確認して承認する、という 1 段階の仕組みだけが用意されています。申請額の大小にかかわらず、承認権限は主任 1 人に集中している状態です。<br>
> ある日、経理部から「決裁権限規程を見直すので、申請額や費目に応じて、権限のある承認者まで自動的に確認が回るようにしてほしい」という依頼が来ました。あなたは以下を担当することになりました。
>
> - 3 万円未満の申請は主任が承認する
> - 10 万円未満の申請は課長が承認する
> - 50 万円未満の申請は部長が承認する
> - 50 万円以上の申請は役員が承認する
> - ただし、費目が「接待交際費」の場合は、金額にかかわらず役員が承認する
...
経費申請の承認処理を行うクラスです。現時点では、申請の内容にかかわらず主任が承認したとみなす処理のみを行います。

| メソッド  | 引数                     | 戻り値の型 | 説明                                                               |
| --------- | ------------------------ | ---------- | ------------------------------------------------------------------ |
| `approve` | `ExpenseRequest request` | `void`     | 申請の内容にかかわらず、主任が承認したとみなして結果をコンソールに出力する |
...
public class Main {
    public static void main(String[] args) {
        ...
        approvalService.approve(new ExpenseRequest("高橋", 600000, "接待交際費"));
    }
}
```
...
高橋さんの申請（接待交際費・600000円） → 主任が承認しました。
```
...
鎖の最後に「必ず処理できる担当者」を置くか、あるいは今回のように鎖の末端に到達した場合の挙動を明示的に用意しておくかは、Chain of Responsibility パターンを使う際に意識しておきたい設計判断です。
