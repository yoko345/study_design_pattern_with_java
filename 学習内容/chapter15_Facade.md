...
> 複数の処理をまとめて呼び出す必要がある場面で、呼び出す側のコードに「どの順番で処理を呼び出し、途中で失敗したら何をどう元に戻すか」という手順の詳細まで書き込む羽目になり、同じ処理を必要とする呼び出し元が増えるたびに、同様の手順の詳細をいたるところにコピーする羽目になった。
...
次に、`OrderProcessor` クラスの `processOrder` メソッドに、失敗時の取り消し処理を追記すると思いますが、こちらの実装に関して真っ先に思いつくのは、取り消し処理をそのまま `processOrder` メソッドに書き加えていく、という実装ではないでしょうか？
...
- 呼び出し側である `OrderProcessor` クラスの `processOrder` メソッドの中に、処理が通ったときの 3 手順に加えて、失敗パターンごとの取り消し処理（`release`・`refund` メソッドの呼び出しとその順序）がすべて書き込まれており、`processOrder` メソッドの中が長くなり可読性が悪くなる。
...
`OrderFacade` は新たに追加したクラスで、手順自体（受付ログの出力は含んでいない）は好ましくない実装①の `OrderProcessor` クラスにあったものと同じです。フィールドとして、既存の `InventoryService`・`PaymentService`・`ShippingService` クラスを保持しています。また、`placeOrder` メソッドに処理が通ったときの手順と取り消しの手順をまとめています。<br>
...
- 呼び出し側である `OrderProcessor` クラスの `processOrder` メソッドの中身が、受付ログの出力と `OrderFacade` クラスの `placeOrder` メソッドの呼び出しの 2 行だけになり、可読性が向上する。
    - これは、処理が通ったときの 3 手順や失敗パターンごとの取り消し処理が `OrderFacade` クラスの `placeOrder` メソッドに移ったためである。
- 呼び出し側である `OrderProcessor` クラスは在庫確保・決済処理・配送手配それぞれの成功・失敗や取り消しの順序を意識する必要がなくなる。
...
`OrderProcessor` クラスの `processOrder` メソッドの中身の一部を `OrderFacade` クラスの `placeOrder` メソッドに移しただけで、呼び出しの階層が 1 つ増えただけではないのか？
...
> 一時的な障害により処理が滞留してしまった注文をまとめて、通常の注文受付と同じ一連の処理（取り消し処理を含む）を再度呼び出す仕組みが必要になりました。<br>
> ただし、これは新規の注文ではないため、注文受付のログは出力しないようにしてください。
...
- `FailedOrderRetryProcessor` クラスの `retryAll` メソッドに、`OrderProcessor` クラスの `processOrder` メソッドの「受付ログの出力」以外の処理とまったく同じ内容がそのままコピーされてしまっている。
    - このように、今後も同じ注文の処理手順を必要とする呼び出し元が増えるたびに、同様のコピーを持つクラスが量産されてしまう。
- 手順変更が必要になった場合（例えば「取り消し処理の順序を変更する」）、コピーされたすべてのクラスを漏れなく修正しなければならず、修正漏れが発生するリスクが高まる。

<a id="正しい実装2"></a>

## 正しい実装②

好ましくない実装②の問題点は、`OrderProcessor` クラスの手順をそのままコピーしてしまった点にありました。正しい実装①ではこの手順がすでに `OrderFacade` クラスという窓口に集約してあるため、コピーする代わりにこの `OrderFacade` クラスを呼び出すだけで解消でき、`FailedOrderRetryProcessor` クラス側で手順を再実装する必要がなくなります。
...
- 手順に変更が必要になった場合（例えば「取り消し処理の順序を変更する」）、修正箇所が `OrderFacade` クラス 1 つに集約されているため、修正漏れが発生するリスクが低い。
...
- 新しい手順（例えば「配送手配の後にポイントを付与する」）を追加する際は、`placeOrder` メソッドを読み解いて正しい位置に処理を挿入しないといけない手間がある点
- 取り消しの順序を注意深く実装する必要がある点
- 処理の後半で失敗するほど、条件分岐の中身が肥大化していく点

このうち、取り消しの順序管理と条件分岐の肥大化について、実務では以下の考え方で解消することが多いです。

> 「成功した処理の取り消し操作をその都度スタックのようなデータ構造に積んでおき、失敗時にはそのスタックを逆順に取り出しながら取り消しを実行する」という補償トランザクションの考え方（分散処理の世界では Saga パターンとも呼ばれます）

この方式では、取り消し処理の記述が「処理を呼び出し、その取り消し操作を積む」という同じ形の繰り返しになるため、取り消しの順序をコードの各所に手動で書き並べる必要がなくなります。<br>
...
正しい実装①を振り返ると、`OrderProcessor` クラスは `OrderFacade` クラスの `placeOrder` メソッドだけを呼び出しています。これにより、好ましくない実装①にはあった `InventoryService`・`PaymentService`・`ShippingService` クラスを直接呼び出している箇所はなくなりました。そのため、Facade パターンを導入すると、サブシステムクラスへの直接アクセスもできなくなったように思うかもしれません。

しかし、Facade パターンは「サブシステムクラスへの直接アクセスを禁止する」ものではありません。現に、`OrderFacade` クラスを導入した後も、`InventoryService`・`PaymentService`・`ShippingService` クラスは `public` のままであり、必要があれば呼び出し側がこれらのクラスを直接使うこともできます。例えば、「在庫状況のみを事前確認したい」という別の要件が出てきた場合、`OrderFacade` クラスに在庫確認専用のメソッドを新たに追加するよりも、`InventoryService` クラスを直接呼び出す方が、追加するコードもテストも少なく済みます。

ただし、このような直接呼び出しを許容したままにしておくと、`InventoryService`・`PaymentService`・`ShippingService` クラスが `OrderFacade` クラスの保証する在庫確保・決済処理・配送手配とその取り消しの手順を経ずに呼び出されてしまい、整合性が崩れてしまうリスクも生まれます。このような時、「`OrderFacade` クラス経由で注文の一連の処理を行いつつも、`InventoryService`・`PaymentService`・`ShippingService` クラスへは直接アクセスできないようにしたい」という場面も出てくると思います。そのような場合にどうすればよいかを、次に説明します（→ [デフォルトによるアクセス制限](#デフォルトによるアクセス制限)）。
...
Facade パターンが提供するのは、あくまで「よく使う一連の手順をまとめた便利な窓口」であり、細かい制御が必要な場面では、これまでどおり呼び出し側がサブシステムクラスを直接呼び出す選択肢も残されています。<br>
ただし、本項で紹介したようにパッケージを切り分けてアクセス修飾子をデフォルトにすることでサブシステムクラスへの直接アクセスを制限した場合、先述の「在庫状況のみを直接確認する」といった柔軟な呼び出しはできなくなります。<br>
直接アクセスの柔軟性を残すか、窓口経由に統一して整合性を優先するかは、要件に応じた設計判断に依存します。
...
好ましくない実装①を振り返ると、`OrderProcessor` クラスは、`InventoryService`・`PaymentService`・`ShippingService` という 3 つのサブシステムクラスの存在と、その呼び出し順序や取り消しの手順を直接知っている必要がありました。正しい実装①により、`OrderProcessor` クラスは `OrderFacade` クラス 1 つだけを知っていればよくなっています。同様に `FailedOrderRetryProcessor` クラスも `OrderFacade` クラスだけを知っていればよく、複数の呼び出し元がそれぞれサブシステムクラスの詳細を知る必要がなくなっています。
...
正しい実装①の `OrderProcessor` クラスがやり取りするオブジェクトは `OrderFacade` クラスだけになり、`InventoryService` クラスなどのサブシステムクラスの存在を知る必要がなくなりました。Facade パターンは、このデメテルの法則を実践するための設計手段の一つと言えます。
