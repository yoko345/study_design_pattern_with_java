...
- 型の統合により、呼び出し側（`TaskManager`）は `addTask`・`addTaskGroup` のように追加先の型に応じてメソッドを使い分ける必要がなくなり、`addComponent` という同じ操作で両者を追加できる。
...
`getEstimatedHours` メソッドは、**子から親へ**戻り値を渡しながら再帰しています。自分自身の子に対して同じ `getEstimatedHours` メソッドを呼び出しており、子が `TaskGroup` だった場合はその呼び出しの中でさらに `getEstimatedHours` メソッドが呼ばれる「**再帰呼び出し**」という形で、戻り値を親に渡していく構造になっています。
