...
基本的には、「委譲を使ったAdapterパターン」を使用する。
継承よりも委譲を使う方がトラブルが少ないためです。
また、親クラスの内部的な振る舞いを詳しく知らないと、継承を効果的に使うのが難しい場合が多いためです。
→[修正の提案]こちらに関しては、本の内容を丸コピ下だけなので、もう少し自分の言葉になるようにClaudeと壁打ちをする。
