...
> あなたはECサイトの開発チームに所属しています。<br>
> 今までクレジットカード決済のみ対応していましたが、若年層ユーザーの利用率向上を目的に、QRコード決済「サンプルPay」を追加することになりました。<br>
...
ECサイトの決済機能における共通インターフェースであり、全ての決済クラスはこれを実現する必要があります。<br>
...
- SamplePayClient（サンプルPay社から提供された、追加したい変更不可の外部クラス）

サンプルPay社が提供する外部クラスです。<br>
社外のコードのため、変更できません。<br>
既存サービスの `PaymentProcessor` インターフェースとメソッド名が異なるため、そのままでは組み込めない状態です。

| メソッド                  | 動作                                         |
| ------------------------- | -------------------------------------------- |
| `void charge(int yen)`    | 「サンプルPayで {`yen`}円 決済します」を出力 |
| `String getServiceName()` | 「サンプルPay」という名称を返す              |
...
「既存コードの仕様」では「サンプルPay社が提供する外部クラスは変更ができない」ことが記載されています。<br>
...
- サンプルPay社が提供した外部クラスが、自社で把握できないタイミングでバージョンアップされた際、今回の実装コードが上書きされてしまうリスクがある
...
このようにメソッドの処理を他のインスタンスに委ねる実装を「委譲を使ったAdapterパターン」といいます。[^1]
...
結論から先に伝えると、基本的には「委譲を使ったAdapterパターン」を使用するのが良いです。<br>
...
継承を使ったパターンでは、`SamplePayAdapter` がすでに `SamplePayClient` を `extends` しているため、Javaの単一継承の制約から `AppLogger` クラスをさらに `extends` できません。[^2]<br>
...
正しい実装①・②から分かるように、Adapterパターンを使用することで、既存のコードに一切手を加えることなく追加実装を行えます。<br>
また、既存コードはすでにテスト済みのため再テストは不要で、変更が行われたAdapterクラスのみテストを行えばよいことから、テスト範囲を最小限に抑えることができます。
