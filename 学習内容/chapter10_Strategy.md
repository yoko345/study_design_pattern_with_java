...
例えば、本記事のシナリオを Template Method パターンで実装するとしたら、`RouteNavigator` を抽象クラスにし、直線距離の計算から `Route` の生成までの処理の流れ自体を `final` メソッドとして固定します。そのうえで、その `final` メソッドの中から呼び出す、探索方針ごとに異なる部分（距離の倍率・想定平均速度・道路種別など）を抽象メソッドとして宣言し、サブクラスにそれぞれ実装させる、という設計になります。

ただし、両者は「何を差し替えの単位とするか」という実現手段が異なります。<br>
Template Method パターンは、スーパークラスの `final` メソッドによりアルゴリズムの骨組みを固定し、サブクラスが各ステップに対応する抽象メソッドをオーバーライドする構成なので、アルゴリズムの一部分だけを差し替える実現の手段となります。また、差し替えの単位は「サブクラス」のため、インスタンス生成時のクラス名指定によりどのサブクラスを使うかを決めるので、コンパイル時点で固定されることになります。<br>
一方、Strategy パターンは、本記事の `RouteNavigator` クラスのように、`find` メソッドの実装全体を `RouteSearchStrategy` インターフェースの実装クラスに委譲しているので、差し替えの単位は「インスタンス」となります。そのため、コンストラクタに渡すインスタンスを変えるだけで実行時に自由に選択・変更ができます。

つまり、「既存コードに手を加えずにアルゴリズムを差し替え可能にする」という目的は同じですが、実現手段が異なります。Template Method パターンは継承によってサブクラスをコンパイル時に固定するのに対し、Strategy パターンは委譲によってインスタンスを実行時に選択します。
